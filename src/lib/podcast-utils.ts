...
export function extractEpisodeContent(content: string): EpisodeContent {
  const sections: Omit<EpisodeContent, "links" | "hosts" | "guests"> & {
    links: string;
    hosts: string;
    guests: string;
  } = {
  ...
  while ((match = sectionRegex.exec(content)) !== null) {
    ...
    sections[sectionName as keyof typeof sections] = sectionContent.trim();
  }

  return {
    ...sections,
    guests: extractLinks(sections.guests),
    links: extractLinks(sections.links),
    hosts: extractLinks(sections.hosts),
